...
$page_layouts["inversion_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	NOMBRE_PROYECTO - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("NOMBRE_PROYECTO", $data, $keylink);
			$row["NOMBRE_PROYECTO_value"] = $value;
			$format = $pSet->getViewFormat("NOMBRE_PROYECTO");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("NOMBRE_PROYECTO")))
				$class = ' rnr-field-number';
			$row["NOMBRE_PROYECTO_class"] = $class;
	//	TIPO - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("inversion", "detailspreview"));
