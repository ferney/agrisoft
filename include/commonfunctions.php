...
function checkTableName($shortTName, $type=false)
{
	...
	if ("agricola" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("finca" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("hortifruticola" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("inversion" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("legal" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lineas_agrarias" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("pecuario" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("propietario" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("REPORTE_GLOBAL" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_FINCA" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_AGRICOLA" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_PECUARIO" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_VEREDA" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_HORTIFRUTICOLA" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_INVERSION" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_LEGAL" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("REPORTE_LINEAS" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	return false;
}
...
function GetTablesList($pdfMode = false)
{
	$arr = array();
		...
		$arr[]="REPORTE GLOBAL";
		$arr[]="REPORTE FINCA";
		$arr[]="REPORTE AGRICOLA";
		$arr[]="REPORTE PECUARIO";
		$arr[]="REPORTE VEREDA";
		$arr[]="REPORTE HORTIFRUTICOLA";
		$arr[]="REPORTE INVERSION";
		$arr[]="REPORTE LEGAL";
		$arr[]="REPORTE LINEAS";
	return $arr;
}
...
//	default permissions
	if($table=="propietario")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE GLOBAL")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE FINCA")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE AGRICOLA")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE PECUARIO")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE VEREDA")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE HORTIFRUTICOLA")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE INVERSION")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE LEGAL")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="REPORTE LINEAS")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("usuario")
