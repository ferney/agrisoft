...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("agricola" == $this->xml_array['tables'][0])
			$isDB = true;
		if("finca" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hortifruticola" == $this->xml_array['tables'][0])
			$isDB = true;
		if("inversion" == $this->xml_array['tables'][0])
			$isDB = true;
		if("legal" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lineas_agrarias" == $this->xml_array['tables'][0])
			$isDB = true;
		if("pecuario" == $this->xml_array['tables'][0])
			$isDB = true;
		if("propietario" == $this->xml_array['tables'][0])
			$isDB = true;
		if("finca" == $this->xml_array['tables'][0])
			$isDB = true;
		if("finca" == $this->xml_array['tables'][0])
