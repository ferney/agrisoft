...
require_once("include/pecuario_variables.php");
...
$page_layouts["pecuario_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	RAZA - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("RAZA", $data, $keylink);
			$row["RAZA_value"] = $value;
			$format = $pSet->getViewFormat("RAZA");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("RAZA")))
				$class = ' rnr-field-number';
			$row["RAZA_class"] = $class;
	//	U_A - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("U_A", $data, $keylink);
			$row["U_A_value"] = $value;
			$format = $pSet->getViewFormat("U_A");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("U_A")))
				$class = ' rnr-field-number';
			$row["U_A_class"] = $class;
	//	AREA - Number
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("pecuario", "detailspreview"));
