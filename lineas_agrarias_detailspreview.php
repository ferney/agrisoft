...
require_once("include/lineas_agrarias_variables.php");
...
$page_layouts["lineas_agrarias_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	LINEA - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("LINEA", $data, $keylink);
			$row["LINEA_value"] = $value;
			$format = $pSet->getViewFormat("LINEA");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("LINEA")))
				$class = ' rnr-field-number';
			$row["LINEA_class"] = $class;
	//	VARIEDAD - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("VARIEDAD", $data, $keylink);
			$row["VARIEDAD_value"] = $value;
			$format = $pSet->getViewFormat("VARIEDAD");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("VARIEDAD")))
				$class = ' rnr-field-number';
			$row["VARIEDAD_class"] = $class;
	//	AREA - Number
			$viewContainer->recId = $recordsCounter;
	...
	//	PRODUCCION_ANUAL - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("PRODUCCION_ANUAL", $data, $keylink);
			$row["PRODUCCION_ANUAL_value"] = $value;
			$format = $pSet->getViewFormat("PRODUCCION_ANUAL");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("PRODUCCION_ANUAL")))
				$class = ' rnr-field-number';
			$row["PRODUCCION_ANUAL_class"] = $class;
	//	AÑO - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("lineas_agrarias", "detailspreview"));
